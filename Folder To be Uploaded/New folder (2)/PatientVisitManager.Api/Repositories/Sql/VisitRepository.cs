...
public class VisitRepository : BaseRepository, IVisitRepository
{
    ...
    public async Task<int> CreateAsync(Visit v)
    {
        using var conn = CreateConn(); await conn.OpenAsync();
        using var cmd = conn.CreateCommand();
        cmd.CommandText = @"INSERT INTO dbo.Visits (visitType, VisittypeID, visitDuration, visitDate, visitTime, visitFee)
                            OUTPUT INSERTED.visitID VALUES (@T,@TID,@Dur,@Date,@Time,@Fee)";
